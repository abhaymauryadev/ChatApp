...
const ProfilePage = () => {
  ...
  const handleSumbit = async(e)=>{
    e.preventDefault();
    console.log('Submitting profile update:', { name, bio, hasImage: !!selectedImg });
    
    try {
      if (selectedImg) {
        // If image is selected, process it first
        const reader = new FileReader();
        reader.readAsDataURL(selectedImg);
        reader.onload = async () => {
          const base64Image = reader.result;
          console.log('Processing image and updating profile...');
      ...
      <div className='w-5/6 max-w-2xl backdrop-blur-2xl text-gray-300
       border-2 border-gray-600 flex items-center justify-between max-sm:flex-col-reverse rounded-lg'>
        <form onSubmit={handleSumbit} className='flex flex-col gap-5 p-10 flex-1 '>
          ...
          type="text" required placeholder='Full Name' className='p-2 border border-gray-500 rounded-md
          focus:outline-none focus:ring-2 focus:ring-violet-500' />

          <textarea onChange={(e)=>setBio(e.target.value)} value={bio} id=""placeholder='Write profile bio here...' required
